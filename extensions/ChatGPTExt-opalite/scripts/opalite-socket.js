...
(function () {
  ...
  // The ISOLATED world content script has host_permissions for *.chatgpt.com,
  ...
  function setupDownloadInterceptor() {
    window.addEventListener('message', function (event) {
      ...
      // Security: explicit allowlist instead of regex — drop unused 'opalite' source
      var ALLOWED_SOURCES = ['opalite'];
      if (ALLOWED_SOURCES.indexOf(source) === -1) return;
    ...
    window.addEventListener('chatgpt-imagine-created', function (event) {
      if (!socket || !isConnected) return;
      var detail = event.detail && event.detail.imagine;
      if (!detail) return;
      log('Detected chatgpt-imagine-created event');
