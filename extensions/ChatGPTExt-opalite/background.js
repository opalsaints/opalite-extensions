...
var SYNC_EXTENSION_TYPE = "chatgpt";
  ...
  if (["opalite"].includes(t.toLowerCase()) && e?.to === "background") {
    switch (e.type) {
      ...
      case "fetchAsDataUri":
        ...
        // fails. Service worker fetch has different Sec-Fetch-* headers.
        (async () => {
          try {
            const url = e.url;
            ...
            // Only allow fetching from ChatGPT/OpenAI domains
            const parsed = new URL(url);
            if (
              !parsed.hostname.endsWith("chatgpt.com") &&
              !parsed.hostname.endsWith("openai.com") &&
              !parsed.hostname.endsWith("oaidalleapiprodscus.blob.core.windows.net")
            ) {
