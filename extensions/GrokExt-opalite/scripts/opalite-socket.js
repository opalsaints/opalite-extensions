...
(function () {
  ...
  // The ISOLATED world content script has host_permissions for *.grok.com,
  // bypassing CORS and Cloudflare challenges that block MAIN world fetch().
  // This is critical for video URLs from assets.grok.com which return
  // cross-origin-resource-policy: same-origin, blocking MAIN world access.
  function fetchViaContentScript(url) {
    return new Promise(function (resolve, reject) {
      ...
      function handler(event) {
        ...
        // Content script responds with type = "fetchAsDataUri:<id>"
      ...
      // Send to content.js — source must be "AutoGrok" to pass its filter
      window.postMessage({
        source: 'AutoGrok',
        to: 'content',
        type: 'fetchAsDataUri',
      ...
      // Timeout after 60 seconds (videos can be large)
  ...
  function setupDownloadInterceptor() {
    window.addEventListener('message', function (event) {
      ...
      if (type === 'downloadImage') {
        ...
        function sendDownload(data) {
          if (socket && isConnected) {
            ...
            // C1: Use ack callback for delivery confirmation
            ...
            // C1: Queue for retry on reconnect (capped at MAX_PENDING)
        ...
        if (downloadData.url && downloadData.url.startsWith('data:')) {
          ...
          // Non-data-URI: fetch via ISOLATED world content script to bypass CORS/Cloudflare.
          // assets.grok.com returns cross-origin-resource-policy: same-origin which blocks
          // MAIN world fetch(), and the server gets 403 from Cloudflare when trying directly.
      ...
      if (type === 'bulkDownloadImages') {
        ...
        // Convert non-data-URI URLs via content script before sending bulk
        var fetchPromises = mappedItems.map(function (item) {
          if (item.url && !item.url.startsWith('data:')) {
            return fetchViaContentScript(item.url).then(function (dataUri) {
              item.url = dataUri;
              return item;
            }).catch(function () {
              // Fallback: keep raw URL (server will attempt fetch)
              return item;
            });
          }
        ...
        Promise.all(fetchPromises).then(function (resolvedItems) {
          var bulkData = {
            items: resolvedItems,
            source: EXTENSION_TYPE
          };
    ...
    window.addEventListener('grok-imagine-created', function (event) {
      if (!socket || !isConnected) return;
      var detail = event.detail && event.detail.imagine;
      if (!detail) return;
      log('Detected grok-imagine-created event');
