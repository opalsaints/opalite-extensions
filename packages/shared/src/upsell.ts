 ...
 * Opalite Upsell UI
 *
 * Listens for quota-exceeded messages from the socket layer and shows
 * an upgrade modal overlay on the AI platform page.
 *
 * Usage (in a WXT content script):
...
function createModal(data: { used?: number; limit?: number }, server: string): void {
  if (modalShown) return;
  modalShown = true;

  // Security: coerce to integers to prevent any injection
  const used = parseInt(String(data.used), 10) || 0;
  const limit = parseInt(String(data.limit), 10) || 0;
  ...
  `;
  upgradeLink.addEventListener('mouseover', function (this: HTMLAnchorElement) {
    this.style.background = '#6d28d9';
  });
  upgradeLink.addEventListener('mouseout', function (this: HTMLAnchorElement) {
 ...
 */
export function setupUpsellListener(server: string): void {
  window.addEventListener('message', (event: MessageEvent) => {
    if (event.source !== window) return;
    if (!event.data || event.data.type !== 'OPALITE_QUOTA_EXCEEDED') return;
    createModal(event.data, server);
  });
}
